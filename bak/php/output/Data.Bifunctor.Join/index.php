...
$Data_Bifunctor_Join_append = ($GLOBALS['Data_Semigroup_append'])($GLOBALS['Data_Semigroup_semigroupString']);
...
$Data_Bifunctor_Join_showJoin = (function() {
...
$show = ($GLOBALS['Data_Show_show'])($dictShow);
  ...
  $__body = function($v) use ($show) {
    ...
    if (true) {
$x = $__case_0;
return ($GLOBALS['Data_Bifunctor_Join_append'])("(Join ", ($GLOBALS['Data_Bifunctor_Join_append'])(($show)($x), ")"));
} else {
...
$Data_Bifunctor_Join_newtypeJoin = ($GLOBALS['Data_Newtype_Newtype__dollar__Dict'])((object)["Coercible0" => (function() {
...
$Data_Bifunctor_Join_bifunctorJoin = (function() {
  $__fn = function($dictBifunctor) use (&$__fn) {
  $__num = func_num_args();
  if ($__num < 1) return phpurs_curry_fallback($__fn, func_get_args(), 1);
$bimap = ($GLOBALS['Data_Bifunctor_bimap'])($dictBifunctor);
    $__res = ($GLOBALS['Data_Functor_Functor__dollar__Dict'])((object)["map" => (function() use ($bimap) {
...
$f1 = $__case_0;
$a = $__case_1;
return ($GLOBALS['Data_Bifunctor_Join_Join'])(($bimap)($f1, $f1, $a));
} else {
throw new \Exception("Pattern match failure");
};
  };
...
$Data_Bifunctor_Join_biapplyJoin = (function() {
  $__fn = function($dictBiapply) use (&$__fn) {
  $__num = func_num_args();
  if ($__num < 1) return phpurs_curry_fallback($__fn, func_get_args(), 1);
$biapply = ($GLOBALS['Control_Biapply_biapply'])($dictBiapply);
$bifunctorJoin1 = ($GLOBALS['Data_Bifunctor_Join_bifunctorJoin'])((($dictBiapply)->Bifunctor0)($GLOBALS['Prim_undefined']));
    $__res = ($GLOBALS['Control_Apply_Apply__dollar__Dict'])((object)["apply" => (function() use ($biapply) {
...
$f = $__case_0;
$a = $__case_1;
return ($GLOBALS['Data_Bifunctor_Join_Join'])(($biapply)($f, $a));
} else {
throw new \Exception("Pattern match failure");
};
  };
...
$Data_Bifunctor_Join_biapplicativeJoin = (function() {
  $__fn = function($dictBiapplicative) use (&$__fn) {
  $__num = func_num_args();
  if ($__num < 1) return phpurs_curry_fallback($__fn, func_get_args(), 1);
$bipure = ($GLOBALS['Control_Biapplicative_bipure'])($dictBiapplicative);
$biapplyJoin1 = ($GLOBALS['Data_Bifunctor_Join_biapplyJoin'])((($dictBiapplicative)->Biapply0)($GLOBALS['Prim_undefined']));
    $__res = ($GLOBALS['Control_Applicative_Applicative__dollar__Dict'])((object)["pure" => (function() use ($bipure) {
  $__fn = function($a) use ($bipure, &$__fn) {
  $__num = func_num_args();
  if ($__num < 1) return phpurs_curry_fallback($__fn, func_get_args(), 1);
    $__res = ($GLOBALS['Data_Bifunctor_Join_Join'])(($bipure)($a, $a));
  return $__num > 1 ? $__res(...array_slice(func_get_args(), 1)) : $__res;
  };
  return $__fn;
